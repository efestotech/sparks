...
This document provides step-by-step instructions for deploying the SPARKS Forge on a shared hosting environment (e.g., SiteGround, Bluehost, Namecheap, Aruba, etc.).
...
2.  Fill in the **Citadel Boundary** (Base URL) and **Vault of Hephaestus** (Database) details.
...
## 6. Configure the Automatic Mantice (Cron)
Shared hosting provides a "Cron Jobs" section in the control panel (cPanel/Plesk).
Add a new cron job to run every minute:

**Command**:
```bash
/usr/bin/curl -s "http://your-domain.com/worker.php?key=YOUR_SECRET_SIGIL" > /dev/null 2>&1
```
*(Replace `YOUR_SECRET_SIGIL` with the key generated during setup).*

## 7. PHP Version Check
Make sure your hosting is set to use **PHP 8.1 or higher**. Most panels allow you to select the version under "PHP Selector" or "MultiPHP Manager".
...
1.  **SymLinks Issue**: Open `public/.htaccess` and find `Options +FollowSymLinks`. Change it to `Options +SymLinksIfOwnerMatch`. (I have already done this in the latest files).
...
4.  **PHP Version**: Ensure you are using **PHP 8.1 or 8.2**. PHP 8.0 or lower will not work.
